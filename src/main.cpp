...
void loop()
{
  ...
  if (now - lastDisplayUpdate >= 200) { // Update displays every 200ms (5Hz)
  ...
  if (lastOdometerUpdate > 0) {
    ...
    if (speedKph > 0.1f) { // Only update if moving (avoid drift when stationary)
  ...
  if (now - lastOdometerDisplay >= 500) { // Update odometer display every 500ms (2Hz)
